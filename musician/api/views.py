...
from musician.api.serializers import UserLoginSerializer, UserRegistrationSerializer, AdminLoginSerializer
from musician.api.models import User, MusicianPost
...
class ResetPassword(APIView):
    ...
    def post(self, request):
        ...
        if not user:
            ...
            誠にありがとうございます。<br/>\
            <br/>\
            【Connect Heart】パスワードがリセットされました。。<br/>\
            <br/>\
...
class PostList(APIView):
    ...
    def post(self, request):
        ...
        offset = pageSize * (pageNumber - 1)
        if keywords!="":
            postlist = MusicianPost.objects.filter(Q(title__contains=keywords))[offset:offset+pageSize]
        else:
            postlist = MusicianPost.objects.filter(Q(is_superuser = False))[offset:offset+pageSize]
        status_code = status.HTTP_200_OK
        response = {
            'status code' : status.HTTP_200_OK,
            'users': postlist,
            'totalRecord': postlist.count(),
            'pageCount': (postlist.count() / pageSize) + 1
        }
        return Response(response, status=status_code)

    def get(self, request, id):
        post = get_object_or_404(MusicianPost, id=id)
        post_serialized = serializers.serialize('json', [post])
        status_code = status.HTTP_200_OK
        response = {
            'success':'true',
            'status code':status_code,
            'user':post_serialized
        }
        return Response(response, status=status_code)